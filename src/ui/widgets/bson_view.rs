...
impl<'a, Msg: 'static> Ctx<'a, Msg> {
    ...
    fn render_val(&self, val: &BsonVal, depth: usize, lines: &mut Vec<Element<'static, Msg>>) {
        match val {
            BsonVal::Null => {
                lines.push(self.line(depth, vec![self.tok("null", self.palette.tok_lit)]))
            }
            BsonVal::Bool(b) => {
                lines.push(self.line(depth, vec![self.tok(b.to_string(), self.palette.tok_lit)]))
            }
            BsonVal::Int(n) => {
                lines.push(self.line(depth, vec![self.tok(n.to_string(), self.palette.tok_num)]))
            }
            BsonVal::Float(f) => {
                lines.push(self.line(depth, vec![self.tok(format!("{f}"), self.palette.tok_num)]))
            }
            BsonVal::NumberLong(n) => lines.push(self.line(
                depth,
                vec![self.tok(format!("NumberLong({n})"), self.palette.tok_call)],
            )),
            BsonVal::ObjectId(s) => {
                lines.push(self.line(depth, vec![self.tok(s.clone(), self.palette.tok_call)]))
            }
            BsonVal::IsoDate(s) => {
                lines.push(self.line(depth, vec![self.tok(s.clone(), self.palette.tok_call)]))
            }
            ...
            BsonVal::Array(arr) => {
                ...
                for v in arr {
                    self.render_val(v, depth + 1, lines);
                }
                lines.push(self.line(depth, vec![self.tok("]", self.palette.tok_br)]));
            }
            BsonVal::Doc(doc) => {
    ...
    fn render_kv(
        ...
        depth: usize,
        comma: bool,
        lines: &mut Vec<Element<'static, Msg>>,
    ) {
        let key_str = if needs_quotes(key) {
            format!("\"{}\"", key)
        } else {
            key.to_string()
        };
        ...
        let comma_tok: Option<Element<'static, Msg>> = if comma {
            Some(self.tok(",", self.palette.tok_colon))
        } else {
            None
        };
        ...
        match val {
            BsonVal::Doc(doc) => {
                // Key: { on its own line
                let opening: Vec<Element<'static, Msg>> =
                    vec![key_tok, colon_tok, self.tok("{", self.palette.tok_br)];
                ...
                let mut closing: Vec<Element<'static, Msg>> =
                    vec![self.tok("}", self.palette.tok_br)];
                if let Some(c) = comma_tok {
                    closing.push(c);
                }
                lines.push(self.line(depth, closing));
            }
            BsonVal::Array(arr) if arr.is_empty() => {
                let mut parts: Vec<Element<'static, Msg>> =
                    vec![key_tok, colon_tok, self.tok("[ ]", self.palette.tok_br)];
                if let Some(c) = comma_tok {
                    parts.push(c);
                }
    ...
    fn inline_val(&self, val: &BsonVal, out: &mut Vec<Element<'static, Msg>>) {
        match val {
            ...
            BsonVal::NumberLong(n) => {
                out.push(self.tok(format!("NumberLong({n})"), self.palette.tok_call))
            }
            ...
            BsonVal::Array(arr) => {
                ...
                for (i, v) in arr.iter().enumerate() {
                    ...
                    if i < arr.len() - 1 {
                        out.push(self.tok(", ", self.palette.tok_colon));
                    }
                }
                out.push(self.tok("]", self.palette.tok_br));
            }
