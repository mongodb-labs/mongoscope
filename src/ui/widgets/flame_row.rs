...
pub fn flame_row<Msg: 'static>(
    ...
    let pct = if data.total_ms == 0 {
        3.0f32
    } else {
        ((data.ms as f32 / data.total_ms as f32) * 100.0).max(3.0)
    };

    let fill_pct = (pct as u16).max(1);
    let rest_pct = 100u16.saturating_sub(fill_pct).max(1);

    let fill_label = Color {
        r: palette.accent_fg.r,
        g: palette.accent_fg.g,
        b: palette.accent_fg.b,
        a: 1.0,
    };
    let bg2 = palette.bg2;

    let bar_fill = container(
        text(format!("{}ms", data.ms))
            .size(10)
            .color(fill_label)
    ...
    let docs_str = data
        .docs
        .map(|d| format!("{} docs", format_num(d)))
        .unwrap_or_default();
