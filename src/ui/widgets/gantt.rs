...
pub fn gantt_row<Msg: 'static>(
    phase: GanttPhase,
    palette: &Palette,
    fs: f32,
) -> Element<'static, Msg> {
    let left_pct = if phase.total_ms == 0 {
        0.0f32
    } else {
        (phase.offset_ms as f32 / phase.total_ms as f32 * 100.0).max(0.0)
    };
    let width_pct = if phase.total_ms == 0 {
        1.0f32
    } else {
        (phase.ms as f32 / phase.total_ms as f32 * 100.0).max(1.0)
    };
    ...
    let track_inner = row![
        iced::widget::Space::new(Length::FillPortion(left_u + 1), 18),
        bar.width(Length::FillPortion(bar_u.max(1))),
        iced::widget::Space::new(Length::FillPortion(right_u + 1), 18),
    ];
