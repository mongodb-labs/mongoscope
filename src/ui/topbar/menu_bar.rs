use iced::{widget::{button, text}, Border, Element, Padding};
...
fn menu_btn<Msg: Clone + 'static>(label: &str, msg: Msg, palette: &Palette) -> Element<'static, Msg> {
    let fg = palette.fg;
    let bg_hover = palette.bg_hover;
    let label = label.to_string();

    button(
        text(label).size(12).color(fg).font(iced::Font::MONOSPACE)
    )
    .padding(Padding { top: 4.0, bottom: 4.0, left: 10.0, right: 10.0 })
    .on_press(msg)
