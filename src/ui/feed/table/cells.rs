use crate::{
    ...
    theme::Palette,
    ui::widgets::{
        appdot::appdot, latency_bar::latency_bar, op_badge::op_badge, plan_chip::plan_chip,
    },
};
use iced::{
...
pub fn op_cell<Msg: 'static>(
    entry: &QueryEntry,
    palette: &Palette,
    _fs: f32,
) -> Element<'static, Msg> {
    container(op_badge(&entry.op, palette)).width(80).into()
}

pub fn coll_cell<Msg: 'static>(
    entry: &QueryEntry,
    palette: &Palette,
    fs: f32,
) -> Element<'static, Msg> {
...
pub fn latency_cell<Msg: 'static>(
    entry: &QueryEntry,
    palette: &Palette,
    fs: f32,
) -> Element<'static, Msg> {
