...
use crate::{
    ...
    theme::Palette,
    ui::widgets::op_badge::op_badge,
};

pub fn inspector_header<Msg: Clone + 'static>(entry: &QueryEntry, palette: &Palette, fs: f32) -> Element<'static, Msg> {
    let bg2 = palette.bg2;
    ...
    let action_btn = move |label: &'static str| -> Element<'static, Msg> {
        button(text(label).size(11).color(fg_dim2).font(iced::Font::MONOSPACE))
    ...
    container(
        row![
            ...
            text("shop.").size(fs).color(fg_dim).font(iced::Font::MONOSPACE),
            text(coll).size(fs).color(fg).font(iced::Font::MONOSPACE),
            text(format!(" · {}", app)).size(fs).color(fg_dim).font(iced::Font::MONOSPACE),
            text(format!(" · #{}", id)).size(fs - 1.0).color(fg_dim2).font(iced::Font::MONOSPACE),
        ...
        .padding(Padding { top: 6.0, bottom: 6.0, left: 12.0, right: 6.0 })
    )
    .width(Length::Fill)
