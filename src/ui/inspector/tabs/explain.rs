...
fn action_label<Msg: 'static>(label: &str, active: bool, palette: &Palette, fs: f32) -> Element<'static, Msg> {
    let (bg, fg, border) = if active {
        (palette.bg_sel, palette.fg, palette.accent)
    } else {
        (palette.bg, palette.fg_dim, palette.border)
    };
    container(text(label.to_string()).size(fs).color(fg).font(iced::Font::MONOSPACE))
...
pub fn explain_tab<Msg: 'static>(entry: &QueryEntry, palette: &Palette, fs: f32) -> Element<'static, Msg> {
    ...
    let plan_label = entry.plan.as_ref().map(|p| p.label()).unwrap_or_else(|| "—".to_string());
    ...
    let explainhd = column![
        row![
            row![
                text("winning plan · ").size(fs_small).color(palette.fg_dim).font(iced::Font::MONOSPACE),
                text(plan_label).size(fs_small).color(plan_color).font(iced::Font::MONOSPACE),
            ],
        ...
        .padding(Padding { top: 0.0, bottom: 8.0, left: 0.0, right: 0.0 }),
        separator(border_c),
    ]
    .width(Length::Fill);
    ...
    // ── suggestions card
    let first_key = entry.filter.as_ref()
        .and_then(|f| f.keys().next().cloned())
        .unwrap_or_else(|| "field".to_string());
    let coll = entry.coll.as_str().to_string();
    let docs_ex = entry.docs_examined.as_ref().map(|d| d.into_inner()).unwrap_or(0);
    let docs_ret = entry.docs_returned.as_ref().map(|d| d.into_inner()).unwrap_or(0);
    let index_label = entry.index.as_ref().map(|i| i.as_str().to_string())
    ...
    let border2 = Color { r: border.r, g: border.g, b: border.b, a: 0.7 };

    let sugg_content: Element<'static, Msg> = if is_bad {
        column![
            text("SUGGESTIONS").size(9).color(fg_dim2).font(iced::Font::MONOSPACE),
            iced::widget::Space::new(0, 6),
            // suggestion 1
            row![
                text("create index").size(fs_small).color(accent).font(iced::Font::MONOSPACE).width(110),
                text(format!("db.{}.createIndex({{ {}: 1 }})", coll, first_key))
                    .size(fs_small).color(fg_dim).font(iced::Font::MONOSPACE).width(Length::Fill),
                text("est. ~4ms · 99.9% faster").size(10).color(fg_dim2).font(iced::Font::MONOSPACE),
            ].spacing(10).align_y(iced::Alignment::Center),
            container(iced::widget::Space::new(Length::Fill, 0))
                .height(1).width(Length::Fill)
                .style(move |_| container::Style {
            ...
            // suggestion 2
            row![
                text("add covered projection").size(fs_small).color(accent).font(iced::Font::MONOSPACE).width(110),
                text(".project({ _id: 0, … })")
                    .size(fs_small).color(fg_dim).font(iced::Font::MONOSPACE).width(Length::Fill),
                text("avoid FETCH stage").size(10).color(fg_dim2).font(iced::Font::MONOSPACE),
            ].spacing(10).align_y(iced::Alignment::Center),
        ]
        .spacing(6)
        .into()
        ...
        column![
            ...
            iced::widget::Space::new(0, 6),
            row![
                text("looks healthy").size(fs_small).color(palette.ok).font(iced::Font::MONOSPACE).width(110),
                text(format!(
...
fn explain_stages(entry: &QueryEntry, total_ms: u32) -> Vec<FlameRowData> {
    ...
    match &entry.plan {
        Some(Plan::CollScan) => vec![
            FlameRowData { name: "COLLSCAN".into(), ms: (total_ms as f32 * 0.92) as u32, total_ms, docs: entry.docs_examined.as_ref().map(|d| d.into_inner()), note: Some("no index usable".into()), kind: FlameRowKind::Bad },
            FlameRowData { name: "SORT (in memory)".into(), ms: (total_ms as f32 * 0.06) as u32, total_ms, docs: entry.docs_examined.as_ref().map(|d| d.into_inner()), note: Some("spill risk".into()), kind: FlameRowKind::Warn },
            FlameRowData { name: "LIMIT".into(), ms: 1, total_ms, docs: entry.docs_returned.as_ref().map(|d| d.into_inner()), note: None, kind: FlameRowKind::Ok },
        ],
        Some(Plan::IxScan(idx)) => vec![
            FlameRowData { name: format!("IXSCAN · {}", idx.as_str()), ms: (total_ms as f32 * 0.35) as u32, total_ms, docs: None, note: None, kind: FlameRowKind::Ok },
            FlameRowData { name: "FETCH".into(), ms: (total_ms as f32 * 0.45) as u32, total_ms, docs: entry.docs_examined.as_ref().map(|d| d.into_inner()), note: None, kind: slow_kind },
        ...
        Some(Plan::IdHack) => vec![
            FlameRowData { name: "IDHACK".into(), ms: total_ms, total_ms, docs: Some(1), note: Some("single-doc by _id".into()), kind: FlameRowKind::Ok },
        ],
        _ => vec![
