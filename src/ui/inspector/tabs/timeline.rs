...
fn mock_neighbours(entry: &QueryEntry) -> Vec<Neighbour> {
    ...
    vec![
        Neighbour {
            t_str: format!("+{}", t.saturating_sub(step * 3)),
            latency: (lat / 3).max(1),
            label: "find · users".into(),
            is_self: false,
        },
        Neighbour {
            t_str: format!("+{}", t.saturating_sub(step * 2)),
            latency: (lat / 2).max(1),
            label: "agg · events".into(),
            is_self: false,
        },
        Neighbour {
            t_str: format!("+{}", t.saturating_sub(step)),
            latency: lat * 2,
            label: format!("find · {}", coll),
            is_self: false,
        },
        Neighbour {
            t_str: format!("+{}", t),
            latency: lat,
            label: format!("{} · {}", op, coll),
            is_self: true,
        },
        Neighbour {
            t_str: format!("+{}", t + step),
            latency: (lat / 4).max(1),
            label: "ins · orders".into(),
            is_self: false,
        },
        Neighbour {
            t_str: format!("+{}", t + step * 2),
            latency: lat * 3 / 4,
            label: "find · products".into(),
            is_self: false,
        },
        Neighbour {
            t_str: format!("+{}", t + step * 3),
            latency: (lat / 5).max(1),
            label: "upd · users".into(),
            is_self: false,
        },
    ]
}

pub fn timeline_tab<Msg: 'static>(
    entry: &QueryEntry,
    palette: &Palette,
    fs: f32,
) -> Element<'static, Msg> {
    ...
    for nbr in neighbours {
        ...
        let bar_pct = ((lat as f32 + 1.0).log10() * 60.0).clamp(8.0, 280.0) / 280.0;
        ...
        let t_str = nbr.t_str;
        let label = nbr.label;
        let lat_str = if lat >= 1000 {
            format!("{:.1}s", lat as f32 / 1000.0)
        } else {
            format!("{}ms", lat)
        };
...
fn mock_phases(total_ms: u32) -> Vec<GanttPhase> {
    ...
    fracs
        .iter()
        .map(|(label, start, end, rgb)| GanttPhase {
