// Tab hidden pending implementation — https://github.com/mongodb-labs/mongoscope/issues/32
#![allow(dead_code)]
...
fn pill_toggle<Msg: Clone + 'static>(
    enabled: bool,
    msg: Msg,
    palette: &Palette,
) -> Element<'static, Msg> {
    ...
    let inner = if enabled {
        row![iced::widget::Space::new(Length::Fill, 0), thumb].padding(Padding {
            top: 2.0,
            bottom: 2.0,
            left: 2.0,
            right: 2.0,
        ...
        row![thumb, iced::widget::Space::new(Length::Fill, 0)].padding(Padding {
            top: 2.0,
            bottom: 2.0,
            left: 2.0,
            right: 2.0,
...
pub fn rules_tab<Msg: Clone + 'static>(
    ...
    let rulehd = row![
        text(format!(
            "{} active rules · matched {}× this session",
            active_count, total_hits
        ))
        .size(fs_small)
        .color(fg_dim)
    ...
    // ── rule rows
    let mut rule_rows: Vec<Element<Msg>> = rules
        .iter()
        .map(|r| {
            let id = r.id;
    ...
    scrollable(column(children).spacing(6).padding(Padding {
        top: 14.0,
        bottom: 14.0,
        left: 16.0,
        right: 16.0,
