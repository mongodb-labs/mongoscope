use iced::{widget::{button, column, container, row, text, text_editor}, Border, Color, Element, Length, Padding};
...
impl ComposeState {
    ...
    pub fn view<'a, Msg: Clone + 'static>(
        ...
        fn ghost_act<Msg: 'static>(label: &str, fg: Color, bg: Color, border: Color, fs: f32) -> Element<'static, Msg> {
        ...
        let composehd = container(
            row![
                row![
                    text("replay on ").size(fs_small).color(fg_dim).font(iced::Font::MONOSPACE),
                    text("prod-cluster-0").size(fs_small).color(accent).font(iced::Font::MONOSPACE),
                    text(" · or switch →").size(fs_small).color(fg_dim).font(iced::Font::MONOSPACE),
                ],
                ...
                row![
                    ...
                    button(
                        text("▶ Run (⌘↵)").size(fs_small).color(accent_fg).font(iced::Font::MONOSPACE)
                    )
                    .padding(Padding { top: 3.0, bottom: 3.0, left: 10.0, right: 10.0 })
        ...
        let composefoot = container(
            row![
                text("shell · mongosh 2.4.0").size(10).color(fg_dim2).font(iced::Font::MONOSPACE),
                iced::widget::Space::new(Length::Fill, 0),
                text("↑↓ history · ⌘K palette · ⌘↵ run").size(10).color(fg_dim2).font(iced::Font::MONOSPACE),
            ]
